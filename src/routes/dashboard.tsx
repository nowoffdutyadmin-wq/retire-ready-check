...
function Dashboard() {
  ...
  async function loadDashboard() {
    ...
    const loadError = content.error || progress.error || streak.error || logs.error || allPairMembers.error;
    ...
    setState({
      ...
      unlockedContentIds: new Set((progress.data ?? []).map((row: { content_id: string }) => row.content_id)),
  ...
  const current = useMemo(() => {
    ...
    return (
      state.content.find((item) => state.unlockedContentIds.has(item.id) && !jointCompleted(state, item.id)) ??
    ...
    return (
      <PortalFrame>
        <section className="mx-auto max-w-3xl rounded-[8px] border bg-card p-6">
          ...
          <p className="mt-4 text-[18px] text-muted-foreground">
            You are in the cohort, but your accountability pair is not assigned yet. Once Chris
            runs auto-pairing, today&apos;s practice will appear here.
          </p>
        </section>
      </PortalFrame>
    );
  ...
  const buddyDone = state.buddies.every((buddy) => current && hasCompleted(state, buddy.id, current.id));
    ...
    <PortalFrame>
      <section className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
        <div>
          ...
          <p className="mt-4 text-[20px] text-muted-foreground">
            {current ? `Day ${current.day_number} - Week ${current.week_number}` : "Content is being prepared"}
          </p>
        </div>
        <div className="rounded-[8px] border bg-card p-6">
          <div className="flex items-center gap-3 text-primary">
            ...
            <div>
              <div className="text-[44px] font-semibold leading-none">{state.streak?.current_streak ?? 0}</div>
      ...
      <section className="mt-8 rounded-[8px] border bg-card p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 font-serif text-[40px] leading-tight">{current?.title ?? "Waiting for audio"}</h2>
            {current && <p className="mt-1 text-[17px] text-muted-foreground">{formatDuration(current.duration_seconds)}</p>}
          </div>
        ...
        <div className="mt-6">
          {current && unlocked ? (
            ...
            <div className="rounded-[8px] bg-secondary p-5 text-[18px]">
              ...
              Waiting for {state.buddies.map((buddy) => firstName(buddy.full_name)).join(" and ")}. Your next session opens when your pair finishes together.
            </div>
          )}
          {current && memberDone && !buddyDone && (
            <p className="mt-4 text-[17px] text-muted-foreground">
              You are complete for today. Waiting for {state.buddies.map((buddy) => firstName(buddy.full_name)).join(" and ")}.
            </p>
          )}
          {current && !memberDone && buddyDone && (
            <p className="mt-4 text-[17px] text-primary">
              {state.buddies.map((buddy) => firstName(buddy.full_name)).join(" and ")} already completed. Your session is ready.
            </p>
          )}
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-2">
            ...
            {state.buddies.map((buddy) => (
              <div key={buddy.id} className="flex items-center justify-between gap-3 text-[17px]">
                <div>
                  ...
                  <div className="text-muted-foreground">{buddy.timezone} - {timezoneLabel(buddy.timezone)}</div>
                </div>
        ...
        <div className="rounded-[8px] border bg-card p-6">
          ...
          <p className="mt-2 text-[17px] text-muted-foreground">{completedDays} of 28 sessions completed by you.</p>
          <div className="mt-6 h-4 overflow-hidden rounded-full bg-secondary">
            <div className="h-full bg-primary" style={{ width: `${Math.min(100, (completedDays / 28) * 100)}%` }} />
          </div>
...
function jointCompleted(state: DashboardState, contentId: string) {
  return [state.member, ...state.buddies].every((member) => hasCompleted(state, member.id, contentId));
}
      ...
      <section className="mx-auto max-w-xl rounded-[8px] border bg-card p-6">
        ...
        <form
          ...
          onSubmit={async (event) => {
            ...
            setSaving(true);
            await onSave(fullName, timezone, password);
            setSaving(false);
          }}
        >
